...
log "pipeline start (cohort=${COHORT:-a})"
...
if ! python3 -m pipeline.collect --cohort "${COHORT:-a}" 2>&1 | tee -a "$LOG_FILE"; then
  log "collect.py failed; refusing to push"
  exit 2
fi

# 3. Validate: the payload must be non-empty JSON with rows.
...
if not data.get("rows"):
    print("[validate] payload has zero rows"); sys.exit(4)
if data.get("meta", {}).get("source") != "confirmed":
    print(f"[validate] meta.source != 'confirmed' (got {data['meta'].get('source')!r})"); sys.exit(5)
print(f"[validate] OK — {len(data['rows'])} rows, generated_at={data['meta']['generated_at']}")
PY
